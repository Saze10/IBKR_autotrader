...
void TestCppClient::tickDataOperation()
{
    ...
    m_pClient->reqMktData(1001, ContractSamples::USDCADcfd(), "", false, false, TagValueListSPtr());
...
void TestCppClient::historicalDataRequests()
{
	...
	m_pClient->reqHistoricalData(4001, ContractSamples::USDCADcash(), "", "30 D", "5 secs", "MIDPOINT", 0, 2, true, TagValueListSPtr());
...
void TestCppClient::calculateIndicators(std::string indicator) {
    if(indicator == "Moving Avg.") {
...
void TestCppClient::orderOperations()
{
    ...
    //ExecutionCondition* execCondition = dynamic_cast<ExecutionCondition *>(OrderSamples::Execution_Condition("CAD.USD", "CASH", "IDEALPRO", true));

    float MD_1 = MacD(24);
    float MD_2 = MacD(60);
    printf("MacD_1: %f\n", MD_1);
    printf("MacD_2: %f\n", MD_2);
    if(MD_1 >= MD_2 + 0.001 && MD_1 <= MD_2 +0.002) {
        Order trailing = OrderSamples::TrailingStop("BUY", 25000, 0.05, 1.3);
        //trailing.conditions.push_back(std::shared_ptr<ExecutionCondition>(execCondition));

        m_pClient->placeOrder(m_orderId++, ContractSamples::USDCADcfd(), trailing);
        //! [order_submission]
        printf("buy order placed\n");
    } else if(MD_1 <= MD_2 - 0.001 && MD_1 >= MD_2-0.002) {
        Order trailing = OrderSamples::TrailingStop("SELL", 25000, 0.05, 1.3);
        //trailing.conditions.push_back(std::shared_ptr<ExecutionCondition>(execCondition));
...
{
     //! [reqhistoricalticks]
     m_pClient->reqHistoricalTicks(19001, ContractSamples::USDCADcash(), "20170621 09:38:33", "", 1000, "BID_ASK", 1, true, TagValueListSPtr());
     m_pClient->reqHistoricalTicks(19002, ContractSamples::USDCADcash(), "20170621 09:38:33", "", 1000, "MIDPOINT", 1, true, TagValueListSPtr());
     m_pClient->reqHistoricalTicks(19003, ContractSamples::USDCADcash(), "20170621 09:38:33", "", 1000, "TRADES", 1, true, TagValueListSPtr());
     //! [reqhistoricalticks]
...
void TestCppClient::historicalData(TickerId reqId, const Bar& bar) {
    ...
    FILE* outfile;
    outfile = fopen("historicaldata.txt", "a");
    fprintf(outfile, "Open:%g,High:%g,Low:%g,Close:%g\n", bar.open, bar.high, bar.low, bar.close);
    fclose(outfile);
}
//! [historicaldata]
